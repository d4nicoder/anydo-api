...
function task({title, categoryId, dueDate}) {
    // if dueDate is not defined
    // it will be set to today
    if (dueDate !== undefined) {
        if (typeof dueDate === 'string') {
            if (_getDueDate(dueDate) === undefined) {
    ...
    // if categoryId is not provided as string
    // it will be set to empty string and the
    // task will be saved into default category
    if (typeof categoryId !== 'string') {
        categoryId = ''
    }

    return {title, dueDate, categoryId, id: _generateId()};
}
